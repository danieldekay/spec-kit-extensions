...
The generated `dod.yml` is the contract between the spec and the implementation. It is updated `speckit.fleet.dod-validate` and exported to specfact by `speckit.fleet.dod-export`.
...
ready_for_sprint:
  ...
  status: "pending"  (speckit.fleet.dod-validate will evaluate and update this)
  checks:
...
GATE STATUS
  ready_for_sprint:    pending  (run speckit.fleet.dod-validate to evaluate)
  definition_of_done:  pending

NEXT STEPS
  1. Review dod.yml — edit criteria descriptions to sharpen testability
  2. Run speckit.fleet.dod-validate after implementation to update statuses
  3. Run speckit.fleet.dod-export to produce specfact-compatible JSON
```
