  ...
  IF FR status is ❌ MISSING:
    FOR each criterion in dod.yml FR:
      IF criterion.status is "pending":
        ...
        APPEND evidence.notes: "Not implemented per speckit.fleet.validate-requirements"
...
```
PRINT: "🔗 sync_after_validate is enabled — running speckit.fleet.specfact-sync..."
RUN speckit.fleet.specfact-sync
```

Otherwise:
```
💡 To push quality results to specfact:  /speckit.fleet.specfact-sync
💡 To update DoD enforcement gate:       /speckit.fleet.dod-export  (bundled with fleet)
```
